...
call actualizar_cliente(2, 'dirección nueva 1', '5269871', 'correonuevo@gmail.com');
...
call eliminar_cuenta_bancaria(9);
select * from cuentas_bancarias;
...
as $$
	...
	-- se valida cuentas
	   ...
	   insert into transacciones(cuenta_id, tipo_transaccion, monto, fecha_transaccion, descripcion) 
	   values(id_cuenta_destino, 'deposito', valor_transferencia, current_timestamp, concepto);
...
call tranferir_saldo_cuentas(6, 4, 1000, 'retiro');
...
as $$
	...
	-- se valida cuentas
	   ...
	   update cuentas_bancarias set saldo = cast(saldo as numeric(15, 2)) - valor_transferencia where cuenta_id = id_cuenta;  
       else
	   update cuentas_bancarias set saldo = cast(saldo as numeric(15, 2)) + valor_transferencia where cuenta_id = id_cuenta;  
	   end if;
	end;
...
call registrar_transaccion(6, 1500000, 'depósito', 'Pago Apartamento');
...
select calcular_saldo_cliente(8);
SELECT * FROM cuentas_bancarias;
...
select * from calcular_saldo_total('2022-01-01', '2024-08-08');
select * from transacciones;